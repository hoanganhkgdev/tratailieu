...
class BulkImportTemplesCommand extends Command
{
    ...
    protected $signature = 'temples:bulk-import
        {path : Đường dẫn thư mục chứa file .docx/.pdf trên server}
        {province : Tên tỉnh/thành (khớp theo tên hoặc alias đã có trong DB)}';

    protected $description = 'Upload hàng loạt file hồ sơ tự viện từ 1 thư mục local lên R2 và đưa vào hàng đợi xử lý AI';
    ...
    public function handle(): int
    {
        ...
        if ($files->isEmpty()) {
            $this->warn('Không có file .docx/.pdf nào trong thư mục này.');
        ...
        $bar->finish();
        $this->newLine(2);
        $this->info("Đã đưa {$created} file vào hàng đợi xử lý".($skipped ? ", bỏ qua {$skipped} file đã xử lý xong trước đó" : '').'.');
        $this->info('Theo dõi tiến trình tại /admin/documents.');
